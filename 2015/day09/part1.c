...
int find_shortest_distance(ht_hash_table* distances, ht_hash_table* graph, char* curr_from, int total_distance, char* visited[100], int visited_count, int total_cities){
    ...
    for(size_t i = 0; i < MAX_CITIES; i++){
        ...
        int res = find_shortest_distance(distances, graph, next_from, total_distance + distance, visited, visited_count+1, total_cities);

        visited[visited_count] = NULL;

        if(res >= 0 && (shortest_distance == -1 || res < shortest_distance)){
...
int main(){
    ...
    while(fgets(buffer,sizeof(buffer),file) != NULL){
        ...
        build_graph(distances, graph, buffer, visited, &total_cities);
    }
    ...
    printf("Total cites: %d\n", total_cities);

    int shortest_distance = find_shortest_distance(distances, graph, start, 0, visited, visited_count, total_cities);
